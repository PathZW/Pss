 ...
 *
 * $Date:        24. May 2018
 * $Revision:    V2.9       modified by linghu0060 (RTE_DEVICE_FRAMEWORK_CLASSIC:
 ...
 * --------------------------------------------------------------------------
 * 1. Disable Automatic Checksum offload: defined Macro EMAC_CHECKSUM_OFFLOAD to 0
 * 2. Enable IEEE 1588 time stamping:     defined Macro EMAC_TIME_STAMP to 1
 * 3. Ethernet interrupt Priority:        defined Macro EMAC_NVIC_PRIO to 1~15
 * 4. Ethernet Memory Buffer:             defined Macro EMAC_RX_BUF_PAD 
 *                                                  and EMAC_TX_BUF_PAD 
 *                                                  and EMAC_RX_BUF_ADDR 
 *                                                  and EMAC_TX_BUF_ADDR  
 *                                                  and EMAC_RX_BUF_NUM 
 *                                                  and EMAC_TX_BUF_NUM
 * 5. Ethernet PHY chip Reset:            defined function void ETH_PHY_Reset(void)
 * -------------------------------------------------------------------------- */
...
/*----------------------------------------------------------------------------*/
#ifndef EMAC_NVIC_PRIO
#define EMAC_NVIC_PRIO      (0x08U)     // Ethernet interrupt Priority
#endif 
...
#ifndef EMAC_RX_BUF_ADDR
#define EMAC_RX_BUF_ADDR                // ETH 接收缓冲区 内存地址定位宏
#endif                                  // -DEMAC_RX_BUF_ADDR=__attribute__((section(\".ARM.__AT_0x2001C000\")))
#ifndef EMAC_TX_BUF_ADDR
#define EMAC_TX_BUF_ADDR                // ETH 发送缓冲区 内存地址定位宏
#endif                                  // -DEMAC_TX_BUF_ADDR=__attribute__((section(\".ARM.__AT_0x2001E400\")))
#ifndef EMAC_RX_BUF_PAD
#define EMAC_RX_BUF_PAD     0U          // ETH 接收缓冲区 填充字节数
#endif
#ifndef EMAC_TX_BUF_PAD
#define EMAC_TX_BUF_PAD     256U        // ETH 发送缓冲区 填充字节数
#endif
#ifndef EMAC_BUF_SIZE
#define EMAC_BUF_SIZE       1536U       // ETH 接收/发送 缓冲区 字节数
#endif
#ifndef EMAC_RX_BUF_NUM
#define EMAC_RX_BUF_NUM     6U          // ETH 接收缓冲区个数; 0x1800 (4*1536=6K)
#endif
#ifndef EMAC_TX_BUF_NUM
#define EMAC_TX_BUF_NUM     4U          // ETH 发送缓冲区个数; 0x0C00 (2*1536=3K)
#endif
/*----------------------------------------------------------------------------*/
...
static void GPIO_Pin_Init(const ETH_PIN *gpio, uint32_t mode)
{
    uint32_t    pos = POSITION_VAL(gpio->pin);
    ...
    MODIFY_REG(gpio->port->OSPEEDR, (((uint32_t)                0x3U) << (2U * pos))
                                  , (((uint32_t)GPIO_SPEED_FREQ_HIGH) << (2U * pos))
              );
    MODIFY_REG(gpio->port->PUPDR, (((uint32_t)       0x3U) << (2U * pos))
                                , (((uint32_t)GPIO_NOPULL) << (2U * pos))
              );
    MODIFY_REG(gpio->port->AFR[pos / 8], (((uint32_t)         0xFU) << (4U * (pos % 8)))
                                       , (((uint32_t)GPIO_AF11_ETH) << (4U * (pos % 8)))
              );
}

static uint32_t ETH_GetTick(void)
{
...
__WEAK void ETH_PHY_Reset(void) {}
/*----------------------------------------------------------------------------*/
...
static RX_Desc   rx_desc[EMAC_RX_BUF_NUM];
static TX_Desc   tx_desc[EMAC_TX_BUF_NUM];
static uint32_t  rx_buf [EMAC_RX_BUF_NUM][(EMAC_BUF_SIZE + EMAC_RX_BUF_PAD) >> 2]  EMAC_RX_BUF_ADDR;
static uint32_t  tx_buf [EMAC_TX_BUF_NUM][(EMAC_BUF_SIZE + EMAC_TX_BUF_PAD) >> 2]  EMAC_TX_BUF_ADDR;
...
static void init_rx_desc (void) {
  ...
  for (i = 0U; i < EMAC_RX_BUF_NUM; i++) {
    rx_desc[i].Stat = DMA_RX_OWN;
    rx_desc[i].Ctrl = DMA_RX_RCH | EMAC_BUF_SIZE;
    rx_desc[i].Addr = (uint8_t *)&rx_buf[i];
    next = i + 1U;
    if (next == EMAC_RX_BUF_NUM) { next = 0U; }
...
static void init_tx_desc (void) {
  ...
  for (i = 0; i < EMAC_TX_BUF_NUM; i++) {
    ...
    next = i + 1U;
    if (next == EMAC_TX_BUF_NUM) { next = 0U; }
...
static int32_t Initialize (ARM_ETH_MAC_SignalEvent_t cb_event) {
  ...
  SYSCFG->PMC &= ~SYSCFG_PMC_MII_RMII_SEL;
    ...
    for (io = eth_pins;  io != &eth_pins[sizeof(eth_pins)/sizeof(ETH_PIN)];  io++) {
      ...
      GPIO_Pin_Init(io, GPIO_MODE_AF_PP);
    }
  #else
    heth.Instance = ETH;
  #endif
...
static int32_t Uninitialize (void) {
  ...
  for (io = eth_pins; io != &eth_pins[sizeof(eth_pins)/sizeof(ETH_PIN)]; io++) {
    GPIO_Pin_DeInit(io->port, io->pin);
  }
#else
  heth.Instance = NULL;
#endif
...
static int32_t PowerControl (ARM_POWER_STATE state) {
  ...
  switch (state) {
    case ARM_POWER_OFF:
      ...
      CLEAR_BIT(RCC->AHB1ENR, RCC_AHB1ENR_ETHMACEN);
      CLEAR_BIT(RCC->AHB1ENR, RCC_AHB1ENR_ETHMACTXEN);
      CLEAR_BIT(RCC->AHB1ENR, RCC_AHB1ENR_ETHMACRXEN);
    #if (EMAC_TIME_STAMP)
      CLEAR_BIT(RCC->AHB1ENR, RCC_AHB1ENR_ETHMACPTPEN);
    #endif
      __NOP(); __NOP(); __NOP(); __NOP();
      #endif
    ...
    case ARM_POWER_FULL:
      ...
      NVIC_SetPriority(ETH_IRQn, EMAC_NVIC_PRIO);
...
static int32_t SendFrame (const uint8_t *frame, uint32_t len, uint32_t flags) {
  ...
  Emac.tx_index++;
  if (Emac.tx_index == EMAC_TX_BUF_NUM) { Emac.tx_index = 0U; }
  Emac.frame_end = NULL;
...
static int32_t ReadFrame (uint8_t *frame, uint32_t len) {
  ...
  Emac.rx_index++;
  if (Emac.rx_index == EMAC_RX_BUF_NUM) { Emac.rx_index = 0; }
...
static int32_t PHY_Read (uint8_t phy_addr, uint8_t reg_addr, uint16_t *data) {
  ...
  } while ((ETH_GetTick() - tick) < PHY_TIMEOUT);
