 ...
 * $Revision:    V2.12      modified by linghu0060 (RTE_DEVICE_FRAMEWORK_CLASSIC:
 *                                      remove the dependency on the HAL library)
 *
 ...
 *
 * 1. USART IO Speed frequencies can be defined by preprocessor:
 ...
 *      default as GPIO_SPEED_FREQ_LOW
 *
 * 2. USART RX and CTS pull-up/pull-down can be defined by preprocessor: (Not ADD)
 *      USARTx_RX_GPIO_PUDP / USARTx_CTS_GPIO_PUDP (where x = 1,2,3 and 6) and 
 *       UARTy_RX_GPIO_PUDP /  UARTy_CTS_GPIO_PUDP (where y = 4,5,7,8,9 and 10)
 *        can be defined as: GPIO_NOPULL, GPIO_PULLUP or GPIO_PULLDOWN
 *      defalut as GPIO_PULLUP
 *
 * 3. USART NVIC Priority can be defined by preprocessor:
 *      USARTx_NVIC_PRIO (where x = 1,2,3 and 6) and 
 *      UARTy_NVIC_PRIO  (where y = 4,5,7,8,9 and 10) can be defined as: 1 to 15
 *      default as 8
 * -------------------------------------------------------------------------- */
...
#define __DMA1_CLK_ENABLE()  (SET_BIT(RCC->AHB1ENR, RCC_AHB1ENR_DMA1EN), __NOP(), __NOP(), __NOP(), __NOP()) // __HAL_RCC_DMA1_CLK_ENABLE()
#define __DMA2_CLK_ENABLE()  (SET_BIT(RCC->AHB1ENR, RCC_AHB1ENR_DMA2EN), __NOP(), __NOP(), __NOP(), __NOP()) // __HAL_RCC_DMA2_CLK_ENABLE()
...
{
    return( SystemCoreClock >> APBPrescTable[(RCC->CFGR & RCC_CFGR_PPRE1) >> 10U] );
}
...
{
    return( SystemCoreClock >> APBPrescTable[(RCC->CFGR & RCC_CFGR_PPRE2) >> 13U] );
}

#ifdef RTE_DEVICE_FRAMEWORK_CLASSIC
/**
 * Initializes the GPIO Pin peripheral
 *
 * @param[in]  gpio   GPIO Pin peripheral
 * @param[in]  mode   GPIO Mode
 */
void GPIO_Pin_Init(const void *DrvGPIO, uint32_t mode)
{
    const USART_PIN  *gpio = DrvGPIO;
    const uint32_t    pos  = POSITION_VAL(gpio->pin);

    MODIFY_REG(gpio->port->MODER, (((uint32_t)            0x3U) << (2U * pos))
                                , (((uint32_t)mode & GPIO_MODE) << (2U * pos))
              );
    MODIFY_REG(gpio->port->OTYPER, (((uint32_t)                                    0x1U ) << (1U * pos))
                                 , (((uint32_t)((mode & GPIO_OUTPUT_TYPE) ? 0x1U : 0x0U)) << (1U * pos))
              );
    MODIFY_REG(gpio->port->OSPEEDR, (((uint32_t)              0x3U) << (2U * pos))
                                  , (((uint32_t)gpio->speed & 0x3U) << (2U * pos))
              );
    MODIFY_REG(gpio->port->PUPDR, (((uint32_t)             0x3U) << (2U * pos))
                                , (((uint32_t)gpio->pupd & 0x3U) << (2U * pos))
              );
    MODIFY_REG(gpio->port->AFR[pos / 8], (((uint32_t)           0xFU) << (4U * (pos % 8)))
                                       , (((uint32_t)gpio->af & 0xFU) << (4U * (pos % 8)))
              );
}
 ...
 * @param[in]  GPIO_Pin  GPIO Pin to be configured
 */
void GPIO_Pin_DeInit(GPIO_TypeDef *GPIOx, uint32_t GPIO_Pin)
{
    uint32_t  pos = POSITION_VAL(GPIO_Pin);
    GPIO_Pin     &= (0x1U << pos);

    if (GPIO_GET_INDEX(GPIOx) == ((SYSCFG->EXTICR[pos / 4] >> (4U * (pos % 4))) & 0x0FU))
    {
        /* Configure the External Interrupt or event for the current IO */
        CLEAR_BIT(SYSCFG->EXTICR[pos / 4], 0x0FU << (4U * (pos % 4)));

        /* Clear EXTI line configuration */
        CLEAR_BIT(EXTI->IMR, GPIO_Pin);
        CLEAR_BIT(EXTI->EMR, GPIO_Pin);

        /* Clear Rising Falling edge configuration */
        CLEAR_BIT(EXTI->RTSR, GPIO_Pin);
        CLEAR_BIT(EXTI->FTSR, GPIO_Pin);
    }

    CLEAR_BIT(GPIOx->MODER       , (0x3U << (2U * (pos    ))));
    CLEAR_BIT(GPIOx->OTYPER      , (0x1U << (1U * (pos    ))));
    CLEAR_BIT(GPIOx->OSPEEDR     , (0x3U << (2U * (pos    ))));
    CLEAR_BIT(GPIOx->PUPDR       , (0x3U << (2U * (pos    ))));
    CLEAR_BIT(GPIOx->AFR[pos / 8], (0xFU << (4U * (pos % 8))));
}
...
{
  #ifdef GPIOA
    if (GPIOx == GPIOA) { SET_BIT(RCC->AHB1ENR, RCC_AHB1ENR_GPIOAEN); }
  #endif
  #ifdef GPIOB
    if (GPIOx == GPIOB) { SET_BIT(RCC->AHB1ENR, RCC_AHB1ENR_GPIOBEN); }
  #endif
  #ifdef GPIOC
    if (GPIOx == GPIOC) { SET_BIT(RCC->AHB1ENR, RCC_AHB1ENR_GPIOCEN); }
  #endif
  #ifdef GPIOD
    if (GPIOx == GPIOD) { SET_BIT(RCC->AHB1ENR, RCC_AHB1ENR_GPIODEN); }
  #endif
  #ifdef GPIOE
    if (GPIOx == GPIOE) { SET_BIT(RCC->AHB1ENR, RCC_AHB1ENR_GPIOEEN); }
  #endif
  #ifdef GPIOF
    if (GPIOx == GPIOF) { SET_BIT(RCC->AHB1ENR, RCC_AHB1ENR_GPIOFEN); }
  #endif
  #ifdef GPIOG
    if (GPIOx == GPIOG) { SET_BIT(RCC->AHB1ENR, RCC_AHB1ENR_GPIOGEN); }
  #endif
  #ifdef GPIOH
    if (GPIOx == GPIOH) { SET_BIT(RCC->AHB1ENR, RCC_AHB1ENR_GPIOHEN); }
  #endif
  #ifdef GPIOI
    if (GPIOx == GPIOI) { SET_BIT(RCC->AHB1ENR, RCC_AHB1ENR_GPIOIEN); }
  #endif
  #ifdef GPIOJ
    if (GPIOx == GPIOJ) { SET_BIT(RCC->AHB1ENR, RCC_AHB1ENR_GPIOJEN); }
  #endif
  #ifdef GPIOK
    if (GPIOx == GPIOK) { SET_BIT(RCC->AHB1ENR, RCC_AHB1ENR_GPIOKEN); }
  #endif
...
static void USART_PeripheralReset (USART_TypeDef *usart) {
  ...
  if      (usart == USART1) { SET_BIT(RCC->APB2RSTR, RCC_APB2RSTR_USART1RST); }
  else if (usart == USART2) { SET_BIT(RCC->APB1RSTR, RCC_APB1RSTR_USART2RST); }
#ifdef USART3 
  else if (usart == USART3) { SET_BIT(RCC->APB1RSTR, RCC_APB1RSTR_USART3RST); }
#endif 
#ifdef UART4 
  else if (usart == UART4)  { SET_BIT(RCC->APB1RSTR, RCC_APB1RSTR_UART4RST);  }
#endif 
#ifdef UART5 
  else if (usart == UART5)  { SET_BIT(RCC->APB1RSTR, RCC_APB1RSTR_UART5RST);  }
#endif 
#ifdef USART6 
  else if (usart == USART6) { SET_BIT(RCC->APB2RSTR, RCC_APB2RSTR_USART6RST); }
#endif 
#ifdef UART7 
  else if (usart == UART7)  { SET_BIT(RCC->APB1RSTR, RCC_APB1RSTR_UART7RST);  }
#endif 
#ifdef UART8 
  else if (usart == UART8)  { SET_BIT(RCC->APB1RSTR, RCC_APB1RSTR_UART8RST);  }
#endif 
#ifdef UART9 
  else if (usart == UART9)  { SET_BIT(RCC->APB2RSTR, RCC_APB2RSTR_UART9RST);  }
#endif 
#ifdef UART10 
  else if (usart == UART10) { SET_BIT(RCC->APB2RSTR, RCC_APB2RSTR_UART10RST); }
#endif 
 
  __NOP(); __NOP(); __NOP(); __NOP(); 
 
  if      (usart == USART1) { CLEAR_BIT(RCC->APB2RSTR, RCC_APB2RSTR_USART1RST); }
  else if (usart == USART2) { CLEAR_BIT(RCC->APB1RSTR, RCC_APB1RSTR_USART2RST); }
#ifdef USART3 
  else if (usart == USART3) { CLEAR_BIT(RCC->APB1RSTR, RCC_APB1RSTR_USART3RST); }
#endif 
#ifdef UART4 
  else if (usart == UART4)  { CLEAR_BIT(RCC->APB1RSTR, RCC_APB1RSTR_UART4RST);  }
#endif 
#ifdef UART5 
  else if (usart == UART5)  { CLEAR_BIT(RCC->APB1RSTR, RCC_APB1RSTR_UART5RST);  }
#endif 
#ifdef USART6 
  else if (usart == USART6) { CLEAR_BIT(RCC->APB2RSTR, RCC_APB2RSTR_USART6RST); }
#endif 
#ifdef UART7 
  else if (usart == UART7)  { CLEAR_BIT(RCC->APB1RSTR, RCC_APB1RSTR_UART7RST);  }
#endif 
#ifdef UART8 
  else if (usart == UART8)  { CLEAR_BIT(RCC->APB1RSTR, RCC_APB1RSTR_UART8RST);  }
#endif 
#ifdef UART9 
  else if (usart == UART9)  { CLEAR_BIT(RCC->APB2RSTR, RCC_APB2RSTR_UART9RST);  }
#endif 
#ifdef UART10 
  else if (usart == UART10) { CLEAR_BIT(RCC->APB2RSTR, RCC_APB2RSTR_UART10RST); }
#endif 
}
...
static int32_t USART_Uninitialize (const USART_RESOURCES *usart) {
  ...
  if (usart->io.tx)  { GPIO_Pin_DeInit(usart->io.tx->port,  usart->io.tx->pin);  }
  if (usart->io.rx)  { GPIO_Pin_DeInit(usart->io.rx->port,  usart->io.rx->pin);  }
  if (usart->io.ck)  { GPIO_Pin_DeInit(usart->io.ck->port,  usart->io.ck->pin);  }
  if (usart->io.rts) { GPIO_Pin_DeInit(usart->io.rts->port, usart->io.rts->pin); }
  if (usart->io.cts) { GPIO_Pin_DeInit(usart->io.cts->port, usart->io.cts->pin); }
    ...
    case ARM_POWER_OFF:
      ...
      if (usart->dma_rx) {
        if (usart->dma_rx->hdma->Instance) {
          ...
          NVIC_DisableIRQ (usart->dma_rx->irq_num);
          // Deinitialize DMA
      ...
      if (usart->dma_tx) {
        if (usart->dma_tx->hdma->Instance) {
          ...
          // Disable DMA IRQ in NVIC
          NVIC_DisableIRQ (usart->dma_tx->irq_num);
          // Deinitialize DMA
...
#ifdef RTE_DEVICE_FRAMEWORK_CLASSIC
      // Disable USART clock
      if      (usart->reg == USART1) { CLEAR_BIT(RCC->APB2ENR, RCC_APB2ENR_USART1EN); }
      else if (usart->reg == USART2) { CLEAR_BIT(RCC->APB1ENR, RCC_APB1ENR_USART2EN); }
    #ifdef USART3 
      else if (usart->reg == USART3) { CLEAR_BIT(RCC->APB1ENR, RCC_APB1ENR_USART3EN); }
    #endif 
    #ifdef UART4 
      else if (usart->reg == UART4)  { CLEAR_BIT(RCC->APB1ENR, RCC_APB1ENR_UART4EN);  }
    #endif 
    #ifdef UART5 
      else if (usart->reg == UART5)  { CLEAR_BIT(RCC->APB1ENR, RCC_APB1ENR_UART5EN);  }
    #endif 
    #ifdef USART6 
      else if (usart->reg == USART6) { CLEAR_BIT(RCC->APB2ENR, RCC_APB2ENR_USART6EN); }
    #endif 
    #ifdef UART7 
      else if (usart->reg == UART7)  { CLEAR_BIT(RCC->APB1ENR, RCC_APB1ENR_UART7EN);  }
    #endif 
    #ifdef UART8 
      else if (usart->reg == UART8)  { CLEAR_BIT(RCC->APB1ENR, RCC_APB1ENR_UART8EN);  }
    #endif 
    #ifdef UART9 
      else if (usart->reg == UART9)  { CLEAR_BIT(RCC->APB2ENR, RCC_APB2ENR_UART9EN);  }
    #endif 
    #ifdef UART10 
      else if (usart->reg == UART10) { CLEAR_BIT(RCC->APB2ENR, RCC_APB2ENR_UART10EN); }
    #endif 
#endif
...
#ifdef RTE_DEVICE_FRAMEWORK_CLASSIC
      // Enable USART clock
      if      (usart->reg == USART1) { SET_BIT(RCC->APB2ENR, RCC_APB2ENR_USART1EN); NVIC_SetPriority(usart->irq_num, USART1_NVIC_PRIO); }
      else if (usart->reg == USART2) { SET_BIT(RCC->APB1ENR, RCC_APB1ENR_USART2EN); NVIC_SetPriority(usart->irq_num, USART2_NVIC_PRIO); }
    #ifdef USART3 
      else if (usart->reg == USART3) { SET_BIT(RCC->APB1ENR, RCC_APB1ENR_USART3EN); NVIC_SetPriority(usart->irq_num, USART3_NVIC_PRIO); }
    #endif 
    #ifdef UART4 
      else if (usart->reg == UART4)  { SET_BIT(RCC->APB1ENR, RCC_APB1ENR_UART4EN);  NVIC_SetPriority(usart->irq_num, UART4_NVIC_PRIO);  }
    #endif 
    #ifdef UART5 
      else if (usart->reg == UART5)  { SET_BIT(RCC->APB1ENR, RCC_APB1ENR_UART5EN);  NVIC_SetPriority(usart->irq_num, UART5_NVIC_PRIO);  }
    #endif 
    #ifdef USART6 
      else if (usart->reg == USART6) { SET_BIT(RCC->APB2ENR, RCC_APB2ENR_USART6EN); NVIC_SetPriority(usart->irq_num, USART6_NVIC_PRIO); }
    #endif 
    #ifdef UART7 
      else if (usart->reg == UART7)  { SET_BIT(RCC->APB1ENR, RCC_APB1ENR_UART7EN);  NVIC_SetPriority(usart->irq_num, UART7_NVIC_PRIO);  }
    #endif 
    #ifdef UART8 
      else if (usart->reg == UART8)  { SET_BIT(RCC->APB1ENR, RCC_APB1ENR_UART8EN);  NVIC_SetPriority(usart->irq_num, UART8_NVIC_PRIO);  }
    #endif 
    #ifdef UART9 
      else if (usart->reg == UART9)  { SET_BIT(RCC->APB2ENR, RCC_APB2ENR_UART9EN);  NVIC_SetPriority(usart->irq_num, UART9_NVIC_PRIO);  }
    #endif 
    #ifdef UART10 
      else if (usart->reg == UART10) { SET_BIT(RCC->APB2ENR, RCC_APB2ENR_UART10EN); NVIC_SetPriority(usart->irq_num, UART10_NVIC_PRIO); }
    #endif 
      __NOP(); __NOP(); __NOP(); __NOP();
...
static int32_t USART_Control (      uint32_t          control,
    ...
    case ARM_USART_CONTROL_TX:
      ...
      if (arg) {
        if (usart->info->mode != ARM_USART_MODE_SMART_CARD) {
          ...
          GPIO_Pin_Init(usart->io.tx, GPIO_MODE_AF_PP);
        }
        ...
        if (usart->info->mode != ARM_USART_MODE_SMART_CARD) {
          ...
          GPIO_Pin_DeInit (usart->io.tx->port, usart->io.tx->pin);
        }
      }
      return ARM_DRIVER_OK;
    ...
    case ARM_USART_CONTROL_RX:
      ...
      if (arg) {
        if ((usart->info->mode != ARM_USART_MODE_SMART_CARD)   &&
          ...
          GPIO_Pin_Init(usart->io.rx, GPIO_MODE_AF_PP);
        }
        ...
        if ((usart->info->mode != ARM_USART_MODE_SMART_CARD)   &&
            (usart->info->mode != ARM_USART_MODE_SINGLE_WIRE )) {
          // GPIO pin function selected
          GPIO_Pin_DeInit (usart->io.rx->port, usart->io.rx->pin);
        }
      }
      return ARM_DRIVER_OK;
    default: break;
  ...
  if (usart->io.ck) {
    switch (usart->info->mode) {
      ...
      case ARM_USART_MODE_SYNCHRONOUS_MASTER:
        ...
        GPIO_Pin_Init(usart->io.ck, GPIO_MODE_AF_PP);
        break;
      default:
        // Asynchronous, Single-wire and IrDA mode
        // GPIO pin function selected
        GPIO_Pin_DeInit (usart->io.ck->port, usart->io.ck->pin);
    }
  }
  ...
  if (usart->io.rts) {
    if ((flow_control == ARM_USART_FLOW_CONTROL_RTS) ||
      ...
      GPIO_Pin_Init(usart->io.rts, GPIO_MODE_AF_PP);
    } else {
      // GPIO output
      GPIO_Pin_Init(usart->io.rts, GPIO_MODE_OUTPUT_PP);
    }
  }
  ...
  if (usart->io.cts) {
    if ((flow_control == ARM_USART_FLOW_CONTROL_CTS) ||
      ...
      GPIO_Pin_Init(usart->io.cts, GPIO_MODE_AF_PP);
    } else {
      // GPIO input
      GPIO_Pin_Init(usart->io.cts, GPIO_MODE_INPUT);
    }
  }
  ...
  switch (control) {
    case ARM_USART_RTS_CLEAR:
      if ((usart->info->flow_control == ARM_USART_FLOW_CONTROL_NONE) ||
        ...
        if (usart->io.rts) {
          GPIO_PIN_SETBIT(usart->io.rts->port, usart->io.rts->pin);
        }
      } else {
        // Hardware RTS
        return ARM_DRIVER_ERROR;
    ...
    case ARM_USART_RTS_SET:
      if ((usart->info->flow_control == ARM_USART_FLOW_CONTROL_NONE) ||
          (usart->info->flow_control == ARM_USART_FLOW_CONTROL_CTS)) {
        if (usart->io.rts) {
          GPIO_PIN_CLRBIT(usart->io.rts->port, usart->io.rts->pin);
        }
      } else {
        // Hardware RTS
        return ARM_DRIVER_ERROR;
...
static ARM_USART_MODEM_STATUS USART_GetModemStatus (const USART_RESOURCES *usart) {
  ...
  if ((usart->info->flow_control == ARM_USART_FLOW_CONTROL_NONE) ||
    ...
    if (usart->io.cts) {
      if (READ_BIT(usart->io.cts->port->IDR, (uint32_t)(usart->io.cts->pin)) == 0) {
        modem_status.cts = 1U;
      }
    }
  }
...
void USART1_TX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_USART1_TX_DMA_IRQn);
...
void USART1_RX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_USART1_RX_DMA_IRQn);
...
void USART2_TX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_USART2_TX_DMA_IRQn);
...
void USART2_RX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_USART2_RX_DMA_IRQn);
...
void USART3_TX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_USART3_TX_DMA_IRQn);
...
void USART3_RX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_USART3_RX_DMA_IRQn);
...
void UART4_TX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_UART4_TX_DMA_IRQn);
...
void UART4_RX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_UART4_RX_DMA_IRQn);
...
void UART5_TX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_UART5_TX_DMA_IRQn);
...
void UART5_RX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_UART5_RX_DMA_IRQn);
...
void USART6_TX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_USART6_TX_DMA_IRQn);
...
void USART6_RX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_USART6_RX_DMA_IRQn);
...
void UART7_TX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_UART7_TX_DMA_IRQn);
...
void UART7_RX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_UART7_RX_DMA_IRQn);
...
void UART8_TX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_UART8_TX_DMA_IRQn);
...
void UART8_RX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_UART8_RX_DMA_IRQn);
...
void UART9_TX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_UART9_TX_DMA_IRQn);
...
void UART9_RX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_UART9_RX_DMA_IRQn);
...
void UART10_TX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_UART10_TX_DMA_IRQn);
...
void UART10_RX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_UART10_RX_DMA_IRQn);
