 ...
 *
 * $Date:        4. May 2020
 * $Revision:    V2.15      modified by linghu0060 (RTE_DEVICE_FRAMEWORK_CLASSIC:
 ...
 *
 * 1. SPI IO Speed frequencies can be defined by preprocessor:
 ...
 *      default as GPIO_SPEED_FREQ_HIGH
 *
 * 2. SPI NSS pull-up/pull-down can be defined by preprocessor: (by linghu0060)
 *      SPIx_NSS_GPIO_PUPD (where x = 1..6) can be defined as
 *        GPIO_NOPULL, GPIO_PULLUP, GPIO_PULLDOWN
 *      default as GPIO_PULLUP
 *
 * 3. SPI NVIC Priority can be defined by preprocessor: (by linghu0060)
 *      SPIx_NVIC_PRIO (where x = 1,2,3 and 6) can be defined as: 1 to 15
 *      default as 8
 * -------------------------------------------------------------------------- */

/* History:
 *  Version 2.15a
 *    Add NVIC_SetPriority() for DMA(modified by linghu0060)
 *  Version 2.15
...
// Macros
#ifdef __SPI_DMA
#define DMA1_CLK_ENABLE()   (SET_BIT(RCC->AHB1ENR, RCC_AHB1ENR_DMA1EN), __NOP(), __NOP(), __NOP(), __NOP()) // __HAL_RCC_DMA1_CLK_ENABLE()
#define DMA2_CLK_ENABLE()   (SET_BIT(RCC->AHB1ENR, RCC_AHB1ENR_DMA2EN), __NOP(), __NOP(), __NOP(), __NOP()) // __HAL_RCC_DMA2_CLK_ENABLE()
#endif
...
*/
static void Enable_SPI_Clock (const SPI_RESOURCES *spi)
{
  if (spi->reg == SPI1)
  {
  #ifdef __SPI_DMA
      if (spi->rx_dma) {
        NVIC_SetPriority((IRQn_Type)spi->rx_dma->irq_num, SPI1_NVIC_PRIO);
      }
      if (spi->tx_dma) {
        NVIC_SetPriority((IRQn_Type)spi->tx_dma->irq_num, SPI1_NVIC_PRIO);
      }
  #endif
      SET_BIT(RCC->APB2ENR, RCC_APB2ENR_SPI1EN);
      NVIC_SetPriority((IRQn_Type)spi->irq_num, SPI1_NVIC_PRIO);
  }
#ifdef SPI2 
  else if (spi->reg == SPI2)
  {
  #ifdef __SPI_DMA
      if (spi->rx_dma) {
        NVIC_SetPriority((IRQn_Type)spi->rx_dma->irq_num, SPI2_NVIC_PRIO);
      }
      if (spi->tx_dma) {
        NVIC_SetPriority((IRQn_Type)spi->tx_dma->irq_num, SPI2_NVIC_PRIO);
      }
  #endif
      SET_BIT(RCC->APB1ENR, RCC_APB1ENR_SPI2EN);
      NVIC_SetPriority((IRQn_Type)spi->irq_num, SPI2_NVIC_PRIO);
  }
#endif 
#ifdef SPI3 
  else if (spi->reg == SPI3)
  {
  #ifdef __SPI_DMA
      if (spi->rx_dma) {
        NVIC_SetPriority((IRQn_Type)spi->rx_dma->irq_num, SPI3_NVIC_PRIO);
      }
      if (spi->tx_dma) {
        NVIC_SetPriority((IRQn_Type)spi->tx_dma->irq_num, SPI3_NVIC_PRIO);
      }
  #endif
      SET_BIT(RCC->APB1ENR, RCC_APB1ENR_SPI3EN);
      NVIC_SetPriority((IRQn_Type)spi->irq_num, SPI3_NVIC_PRIO);
  }
#endif 
#ifdef SPI4 
  else if (spi->reg == SPI4)
  {
  #ifdef __SPI_DMA
      if (spi->rx_dma) {
        NVIC_SetPriority((IRQn_Type)spi->rx_dma->irq_num, SPI4_NVIC_PRIO);
      }
      if (spi->tx_dma) {
        NVIC_SetPriority((IRQn_Type)spi->tx_dma->irq_num, SPI4_NVIC_PRIO);
      }
  #endif
      SET_BIT(RCC->APB2ENR, RCC_APB2ENR_SPI4EN);
      NVIC_SetPriority((IRQn_Type)spi->irq_num, SPI4_NVIC_PRIO);
  }
#endif 
#ifdef SPI5 
  else if (spi->reg == SPI5)
  {
  #ifdef __SPI_DMA
      if (spi->rx_dma) {
        NVIC_SetPriority((IRQn_Type)spi->rx_dma->irq_num, SPI5_NVIC_PRIO);
      }
      if (spi->tx_dma) {
        NVIC_SetPriority((IRQn_Type)spi->tx_dma->irq_num, SPI5_NVIC_PRIO);
      }
  #endif
      SET_BIT(RCC->APB2ENR, RCC_APB2ENR_SPI5EN);
      NVIC_SetPriority((IRQn_Type)spi->irq_num, SPI5_NVIC_PRIO);
  }
#endif 
#ifdef SPI6 
  else if (spi->reg == SPI6)
  {
  #ifdef __SPI_DMA
      if (spi->rx_dma) {
        NVIC_SetPriority((IRQn_Type)spi->rx_dma->irq_num, SPI6_NVIC_PRIO);
      }
      if (spi->tx_dma) {
        NVIC_SetPriority((IRQn_Type)spi->tx_dma->irq_num, SPI6_NVIC_PRIO);
      }
  #endif
      SET_BIT(RCC->APB2ENR, RCC_APB2ENR_SPI6EN);
      NVIC_SetPriority((IRQn_Type)spi->irq_num, SPI6_NVIC_PRIO);
  }
#endif 
...
static void Disable_SPI_Clock (const SPI_TypeDef *spi) {
  if      (spi == SPI1) { CLEAR_BIT(RCC->APB2ENR, RCC_APB2ENR_SPI1EN); }
#ifdef SPI2 
  else if (spi == SPI2) { CLEAR_BIT(RCC->APB1ENR, RCC_APB1ENR_SPI2EN); }
#endif 
#ifdef SPI3 
  else if (spi == SPI3) { CLEAR_BIT(RCC->APB1ENR, RCC_APB1ENR_SPI3EN); }
#endif 
#ifdef SPI4 
  else if (spi == SPI4) { CLEAR_BIT(RCC->APB2ENR, RCC_APB2ENR_SPI4EN); }
#endif 
#ifdef SPI5 
  else if (spi == SPI5) { CLEAR_BIT(RCC->APB2ENR, RCC_APB2ENR_SPI5EN); }
#endif 
#ifdef SPI6 
  else if (spi == SPI6) { CLEAR_BIT(RCC->APB2ENR, RCC_APB2ENR_SPI6EN); }
#endif 
}
#endif
...
static void SPI_PeripheralReset (const SPI_TypeDef *spi) {
  ...
  if      (spi == SPI1) { SET_BIT(RCC->APB2RSTR, RCC_APB2RSTR_SPI1RST); }
#ifdef SPI2 
  else if (spi == SPI2) { SET_BIT(RCC->APB1RSTR, RCC_APB1RSTR_SPI2RST); }
#endif 
#ifdef SPI3 
  else if (spi == SPI3) { SET_BIT(RCC->APB1RSTR, RCC_APB1RSTR_SPI3RST); }
#endif 
#ifdef SPI4 
  else if (spi == SPI4) { SET_BIT(RCC->APB2RSTR, RCC_APB2RSTR_SPI4RST); }
#endif 
#ifdef SPI5 
  else if (spi == SPI5) { SET_BIT(RCC->APB2RSTR, RCC_APB2RSTR_SPI5RST); }
#endif 
#ifdef SPI6 
  else if (spi == SPI6) { SET_BIT(RCC->APB2RSTR, RCC_APB2RSTR_SPI6RST); }
#endif 
 
  __NOP(); __NOP(); __NOP(); __NOP(); 
 
  if      (spi == SPI1) { CLEAR_BIT(RCC->APB2RSTR, RCC_APB2RSTR_SPI1RST); }
#ifdef SPI2 
  else if (spi == SPI2) { CLEAR_BIT(RCC->APB1RSTR, RCC_APB1RSTR_SPI2RST); }
#endif 
#ifdef SPI3 
  else if (spi == SPI3) { CLEAR_BIT(RCC->APB1RSTR, RCC_APB1RSTR_SPI3RST); }
#endif 
#ifdef SPI4 
  else if (spi == SPI4) { CLEAR_BIT(RCC->APB2RSTR, RCC_APB2RSTR_SPI4RST); }
#endif 
#ifdef SPI5 
  else if (spi == SPI5) { CLEAR_BIT(RCC->APB2RSTR, RCC_APB2RSTR_SPI5RST); }
#endif 
#ifdef SPI6 
  else if (spi == SPI6) { CLEAR_BIT(RCC->APB2RSTR, RCC_APB2RSTR_SPI6RST); }
#endif 
}
...
static int32_t SPI_Initialize (ARM_SPI_SignalEvent_t cb_event, const SPI_RESOURCES *spi) {
  ...
  if (spi->io.sck) {
    ...
    GPIO_Pin_Init(spi->io.sck, GPIO_MODE_AF_PP);
  }

  if (spi->io.mosi) {
    // Configure SPI MOSI pin
    Enable_GPIO_Clock (spi->io.mosi->port);
    GPIO_Pin_Init(spi->io.mosi, GPIO_MODE_AF_PP);
  }

  if (spi->io.miso) {
    // Configure SPI MISO pin
    Enable_GPIO_Clock (spi->io.miso->port);
    GPIO_Pin_Init(spi->io.miso, GPIO_MODE_AF_PP);
  }
...
static int32_t SPI_Uninitialize (const SPI_RESOURCES *spi) {
  ...
  if (spi->io.mosi != NULL) { GPIO_Pin_DeInit(spi->io.mosi->port, spi->io.mosi->pin); }
  if (spi->io.miso != NULL) { GPIO_Pin_DeInit(spi->io.miso->port, spi->io.miso->pin); }
  if (spi->io.sck  != NULL) { GPIO_Pin_DeInit(spi->io.sck->port,  spi->io.sck->pin);  }
  if (spi->io.nss  != NULL) { GPIO_Pin_DeInit(spi->io.nss->port,  spi->io.nss->pin);  }
    ...
    case ARM_POWER_OFF:
...
#ifdef RTE_DEVICE_FRAMEWORK_CLASSIC
      NVIC_DisableIRQ ((IRQn_Type)spi->irq_num);
#ifdef __SPI_DMA
      if (spi->rx_dma != NULL) {
        if (spi->rx_dma->hdma->Instance != NULL) {
          // Disable DMA IRQ in NVIC
          NVIC_DisableIRQ ((IRQn_Type)spi->rx_dma->irq_num);
          // Deinitialize DMA
      ...
      if (spi->tx_dma != NULL) {
        if (spi->tx_dma->hdma->Instance != NULL) {
          // Disable DMA IRQ in NVIC
          NVIC_DisableIRQ ((IRQn_Type)spi->tx_dma->irq_num);
          // Deinitialize DMA
      ...
      if (val == ARM_SPI_MODE_MASTER) {
        ...
        if ((spi->io.nss != NULL) && (val == ARM_SPI_SS_MASTER_SW)) {
          ...
          if (arg == ARM_SPI_SS_INACTIVE)
            GPIO_PIN_SETBIT(spi->io.nss->port, spi->io.nss->pin);
          else
            GPIO_PIN_CLRBIT(spi->io.nss->port, spi->io.nss->pin);
  ...
  if (mode == ARM_SPI_MODE_MASTER) {
    switch (control & ARM_SPI_SS_MASTER_MODE_Msk) {
      case ARM_SPI_SS_MASTER_UNUSED:
        if (spi->io.nss != NULL) {
          // Unconfigure NSS pin
          GPIO_Pin_DeInit (spi->io.nss->port, spi->io.nss->pin);
        }
...
#endif
          GPIO_Pin_Init(spi->io.nss, GPIO_MODE_AF_PP);
        } else {
      ...
      case ARM_SPI_SS_MASTER_SW:
        if (spi->io.nss) {
          ...
          Enable_GPIO_Clock (spi->io.nss->port);
#endif
          GPIO_Pin_Init(spi->io.nss, GPIO_MODE_OUTPUT_PP);
...
void SPI1_TX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_SPI1_TX_DMA_IRQn);
...
void SPI1_RX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_SPI1_RX_DMA_IRQn);
...
void SPI2_TX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_SPI2_TX_DMA_IRQn);
...
void SPI2_RX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_SPI2_RX_DMA_IRQn);
...
void SPI3_TX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_SPI3_TX_DMA_IRQn);
...
void SPI3_RX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_SPI3_RX_DMA_IRQn);
...
void SPI4_TX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_SPI4_TX_DMA_IRQn);
...
void SPI4_RX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_SPI4_RX_DMA_IRQn);
...
void SPI5_TX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_SPI5_TX_DMA_IRQn);
...
void SPI5_RX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_SPI5_RX_DMA_IRQn);
...
void SPI6_TX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_SPI6_TX_DMA_IRQn);
...
void SPI6_RX_DMA_Handler (void) {
  NVIC_ClearPendingIRQ(MX_SPI6_RX_DMA_IRQn);
