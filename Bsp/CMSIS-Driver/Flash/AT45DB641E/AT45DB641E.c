...
#define SECTOR_SIZE     (64*BLOCK_SIZE)   /* Sector size */

/* SPI Data Flash Commands */
#define CMD_READ_DATA           (0x1BU)
...
#define CMD_CHIP_ERASE_1        (0xC7U)
#define CMD_CHIP_ERASE_2        (0x94U)
#define CMD_CHIP_ERASE_3        (0x80U)
#define CMD_CHIP_ERASE_4        (0x9AU)
...
static int32_t ReadData (uint32_t addr, void *data, uint32_t cnt) {
  ...
  /* Send Command with Address */
  result = ptrSPI->Send(buf, 6);
  if (result != ARM_DRIVER_OK) { goto transfer_error; }
  while (ptrSPI->GetDataCount() != 6);
...
static int32_t EraseChip (void) {
  ...
  /* Prepare Chip Erase Command */
  buf[0] = CMD_CHIP_ERASE_1;
  buf[1] = CMD_CHIP_ERASE_2;
  buf[2] = CMD_CHIP_ERASE_3;
  buf[3] = CMD_CHIP_ERASE_4;
...
static ARM_FLASH_STATUS GetStatus (void) {
  ...
  if (FlashStatus.busy == 1U) {
    ...
    if (ptrSPI->Control(ARM_SPI_CONTROL_SS, ARM_SPI_SS_ACTIVE) == ARM_DRIVER_OK) {
      /* Send Command */
      if (ptrSPI->Send(buf, 1) == ARM_DRIVER_OK) {
        /* Wait till transfer done */
        while (ptrSPI->GetDataCount() != 1);

        /* Check Status Register */
        if (ptrSPI->Receive(buf, 2) == ARM_DRIVER_OK) {
          /* Wait till transfer done */
          while (ptrSPI->GetDataCount() != 2);
