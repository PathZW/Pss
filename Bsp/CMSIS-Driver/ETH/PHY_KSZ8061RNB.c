 ...
 *  Version 1.2
 *    Reworked driver flow control flags
 *  Version 1.1
...
static ARM_ETH_LINK_INFO GetLinkInfo (void) {
  ...
  if (PHY.flags & PHY_POWER) {
    ...
    PHY.reg_rd(ETH_PHY_ADDR, REG_PHYCR1, &val);
  }

  /* Link must be up to get valid state */
  info.speed  = (val & PHYCR1_OM_100B) ? ARM_ETH_SPEED_100M  : ARM_ETH_SPEED_10M;
  info.duplex = (val & PHYCR1_OM_FD)   ? ARM_ETH_DUPLEX_FULL : ARM_ETH_DUPLEX_HALF;
