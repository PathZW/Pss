...
static int32_t PowerControl (ARM_POWER_STATE state) {
  ...
  switch ((int32_t)state) {
    case ARM_POWER_OFF:
      ...
      PHY.flags &= ~PHY_POWER;
      PHY.cntrl  =  CNTRL_POWER_DOWN;

      return (PHY.reg_wr(ETH_PHY_ADDR, REG_CNTRL, CNTRL_POWER_DOWN));
    ...
    case ARM_POWER_FULL:
      ...
      if (PHY.reg_wr(ETH_PHY_ADDR, REG_CNTRL, PHY.cntrl) != ARM_DRIVER_OK) {
        return ARM_DRIVER_ERROR;
      }
...
static int32_t SetInterface (uint32_t interface) {
  ...
  return (PHY.reg_wr(ETH_PHY_ADDR, REG_XCNTL, val));
}
...
static int32_t SetMode (uint32_t mode) {
  ...
  val = PHY.cntrl & CNTRL_POWER_DOWN;
  ...
  return (PHY.reg_wr(ETH_PHY_ADDR, REG_CNTRL, PHY.cntrl));
}
...
static ARM_ETH_LINK_STATE GetLinkState (void) {
  ...
  if (PHY.flags & PHY_POWER) {
    PHY.reg_rd(ETH_PHY_ADDR, REG_STATS, &val);
  }
...
static ARM_ETH_LINK_INFO GetLinkInfo (void) {
  ...
  if (PHY.flags & PHY_POWER) {
    PHY.reg_rd(ETH_PHY_ADDR, REG_XSTAT, &val);
  }
