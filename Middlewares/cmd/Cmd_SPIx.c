 ...
 *          SPI 驱动: 若不使用 CMSIS-Driver, 可定义 SPIx_INIT() SPIx_SEND_START()
 *                    SPIx_RECV_START(), 并在 SPI 完成中断中调用 SPIx_IntrComplete() \n
 *          NSS 信号: 支持 硬件NSS 及 自定义NSS, 自定义需实现3个 SPIx_NSS_ 开头函数. \n
 *******************************************************************************
 * @attention
 *
 ...
 *
 * @par Change Logs:
 *      2021/8/17 --- Linghu -- Add Event Recorder Execution Statistics D(1, 2, 5, 6)
 *
 * @par Change Logs:
...
#if defined SPIx_NSS_INIT
__WEAK void SPIx_NSS_INIT(void) {}  //lint -esym(522, SPIx_NSS_INIT)
#endif
#if defined SPIx_ACT_LED
__WEAK void SPIx_ACT_LED(void) {}   //lint -esym(522, SPIx_ACT_LED)
#endif
...
#ifndef SPIx_DEVICE_NUM
#define SPIx_DEVICE_NUM             -1          //!< (C) SPI number, ( < 0) Disable
#endif
#ifndef SPIx_BUS_SPEED
#define SPIx_BUS_SPEED           4000000        //!< (C) 总线速度(以 Hz 为单位)
#endif
#ifndef SPIx_BUFF_SIZE
#define SPIx_BUFF_SIZE             256          //!< (C) 驱动缓存(以字节为单位)
#endif

#ifndef SPIx_CMD_STR_EN
#define SPIx_CMD_STR_EN              1          //!< (R) 是(1)/否(0) 支持字符串命令
#endif
#ifndef SPIx_CMD_BIN_EN
#define SPIx_CMD_BIN_EN              0          //!< (R) 是(1)/否(0) 支持二进制命令
#endif
#ifndef SPIx_FORWARD_IDX
#define SPIx_FORWARD_IDX            -1          //!< (R) 转发索引, (< 0)非目的接口
#endif
#ifndef SPIx_FORWARD_SRC
#define SPIx_FORWARD_SRC            -1          //!< (R) 转发索引, (< 0)非默认源接口
#endif

/*----------------------------------------------------------------------------*/
#ifndef SPIx_POLL_TICKS
#define SPIx_POLL_TICKS              2          //!< (R) Master 轮询时间, (< 0)不轮询
#endif

#ifndef SPIx_MODE_MASTER
#define SPIx_MODE_MASTER             1          //!< (C) Master模式(1) 或 Slave模式(0)
#endif
#ifndef SPIx_MODE_SIMPLEX
#define SPIx_MODE_SIMPLEX            0          //!< (C) 半双工(1) 或 全双工(0)
#endif
#ifndef SPIx_MODE_NSS_SW
#define SPIx_MODE_NSS_SW             1          //!< (C) NSS信号; 1: CMSIS SW, 0: Master自定义 | Slave CMSIS HW
#endif
#ifndef SPIx_MODE_CFG
#define SPIx_MODE_CFG    (  ARM_SPI_MSB_LSB     /*!< (C) SPI 工作模式     */ \
                          | ARM_SPI_CPOL0_CPHA0                              \
                          | ARM_SPI_DATA_BITS(8)                             \
                         )
#endif
...
#ifndef SPIx_INIT
#define SPIx_INIT               SPIx_INIT       //!< (C) SPI 初始化 (函数名)
#endif
#ifndef SPIx_SEND_START
#define SPIx_SEND_START      SPIx_SEND_START    //!< (C) SPI 开始发送数据 (函数名)
#endif
#ifndef SPIx_RECV_START
#define SPIx_RECV_START      SPIx_RECV_START    //!< (C) SPI 开始接收数据 (函数名)
#endif

#ifndef SPIx_NSS_INIT
#define SPIx_NSS_INIT()                         //!< (C) SPI NSS 信号初始化 (函数名)
#endif
#ifndef SPIx_NSS_CLR
#define SPIx_NSS_CLR            SPIx_NSS_CLR    //!< (C) SPI NSS 信号置为无效 (函数名)
#endif
#ifndef SPIx_NSS_SET
#define SPIx_NSS_SET            SPIx_NSS_SET    //!< (C) SPI NSS 信号置为有效 (函数名)
#endif

#ifndef SPIx_ACT_LED
#define SPIx_ACT_LED()                          //!< (C) SPI 命令接口激活显示 (函数名)
#endif
...
extern ARM_DRIVER_SPI       ATPASTE2(Driver_SPI, SPIx_DEVICE_NUM);
#define ptrSPI            (&ATPASTE2(Driver_SPI, SPIx_DEVICE_NUM))

#define CMD_CLI_NAME        "SPIx"
...
static QUEUE_TYPE(uint8_t, SPIx_BUFF_SIZE)  _tx_Queue;   // 发送队列
static QUEUE_TYPE(uint8_t, SPIx_BUFF_SIZE)  _rx_Queue;   // 接收队列
 ...
 * @{
 ******************************************************************************/
//lint -esym(522, SPIx_NSS*)
//lint -esym(528, SPIx_NSS*)

#ifdef SPIx_INIT
__WEAK int SPIx_INIT(void)
{
    uint32_t    control;

#if SPIx_MODE_MASTER            /*lint -save -w1 */
  #if SPIx_MODE_SIMPLEX
  ...
  #endif
#endif // SPIx_MODE_MASTER
    control |= SPIx_MODE_CFG;   /*lint -restore */
    ...
    if (ptrSPI->Control(control, SPIx_BUS_SPEED) != ARM_DRIVER_OK) {
        LOG_ERROR("ptrSPI->Control(SPIx_MODE_CFG, SPIx_BUS_SPEED)");
        return( -1 );
    }
...
__WEAK int SPIx_SEND_START(const void *data, unsigned num)
{
    return( ptrSPI->Send(data, num) );
}


__WEAK int SPIx_RECV_START(void *data, unsigned num)
{
...
__WEAK void SPIx_NSS_CLR(void)
{
#if SPIx_MODE_NSS_SW    // NSS信号为 CMSIS-Driver Software 模式
...
void SPIx_IntrComplete(void)
{
    ...
    case CMD_SPI_NEGO_MO_TX :   // -------- 协商长度(Master 发送) ------------
        ...
        SPIx_RECV_START(_neg_Buf, ARRAY_SIZE(_neg_Buf)); //lint !e534   // 开始接收协商
        break;
    ...
    case CMD_SPI_NEGO_MO_RX :   // -------- 协商长度(Master 接收) ------------
        ...
        SPIx_SEND_START(_buff, _size); //lint !e534                     // 开始发送数据
        break;
    ...
    case CMD_SPI_NEGO_MI_RX :   // -------- 协商长度(Master 接收) ------------
        ...
        SPIx_RECV_START(_buff, _size); //lint !e534                     // 开始接收数据
        break;
    ...
    case CMD_SPI_DATA_MO_TX :   // -------- 传输数据(Master 发送) ------------
      ...
      #ifdef RTE_Compiler_EventRecorder
        EventStopDx(1, "__SEND_STOP__", _size);      //lint !e534 !e737 !e835
      #endif
    ...
    case CMD_SPI_DATA_MI_RX :   // -------- 传输数据(Master 接收) ------------
      ...
      #ifdef RTE_Compiler_EventRecorder
        EventStopDx(2, "__RECV_STOP__", _size);      //lint !e534 !e737 !e835
      #endif
        SPI_MasterStart((void *)CMD_SPI_OPT_MISO);                      // Master 接收启动
        break;
    ...
    case CMD_SPI_NEGO_RX :      // -------- 协商长度(Slave 接收) ------------
        ...
        SPIx_SEND_START(_neg_Buf, ARRAY_SIZE(_neg_Buf)); //lint !e534   // 开始发送协商
...
static void SPI_MasterStart(void *arg)
{
    ...
    case CMD_SPI_OPT_MOSI : // ------------ 发送优先 --------------------
        if((len = QUEUE_O_BUF(_tx_Queue, &buff)) > 0) {
          ...
          #ifdef RTE_Compiler_EventRecorder
            EventStartDx(1, buff, len); //lint !e534 !e737 !e835
          #endif
        } else
        if((len = QUEUE_I_BUF(_rx_Queue, &buff)) > 0) {
          ...
          #ifdef RTE_Compiler_EventRecorder
            EventStartDx(2, "__RECV_START__", len); //lint !e534 !e737 !e835
          #endif
        } else {
    ...
    SPIx_NSS_SET();                         // NSS 信号置为有效
    _neg_Buf[0] = (HIBYTE(len) & CMD_SPI_LEN_MASK) | opt;
    _neg_Buf[1] =  LOBYTE(len);             // 开始发送协商
    SPIx_SEND_START(_neg_Buf, ARRAY_SIZE(_neg_Buf)); //lint !e534
}
#else
/**
 * Slave 通信启动
...
__STATIC_FORCEINLINE void SPI_SlaveStart(void)
{
    ...
    SPIx_NSS_SET();         // note         // NSS 信号置为有效
    _neg_Buf[0] = CMD_SPI_OPT_NONE,  _neg_Buf[1] = 0;
    SPIx_RECV_START(_neg_Buf, ARRAY_SIZE(_neg_Buf)); //lint !e534
}
#endif  // (SPIx_MODE_MASTER != 0)
...
static void SPI_SignalEvent(uint32_t event)
{
    ...
    if (event & ARM_SPI_EVENT_DATA_LOST) {
        LOG_WARN("SPI" TO_STRING(SPIx_DEVICE_NUM) " data lost: %u", (unsigned)_spi_Stat);
    }
    if (event & ARM_SPI_EVENT_MODE_FAULT) {
        LOG_WARN("SPI" TO_STRING(SPIx_DEVICE_NUM) " Master mode NSS fault");
    }
}
...
{
    if (SPIx_INIT() != 0) { // SPI 初始化
        return( -1 );
    }
    SPIx_NSS_INIT();        // SPI NSS 信号初始化
    SPIx_NSS_CLR();         // NSS 信号置为无效
    ...
    {   osTimerId_t     tmrID;
        ...
        if (osTimerStart(tmrID, SPIx_POLL_TICKS) != osOK) {
            LOG_ERROR("osTimerStart(tmrID, SPIx_POLL_TICKS)");
...
#ifdef RTE_Compiler_EventRecorder
    EventStartDx(5, data, size); //lint !e534 !e737 !e835
#endif
...
#ifdef RTE_Compiler_EventRecorder
    EventStopDx(6, data, size);  //lint !e534 !e737 !e835
#endif
    LOG_VERB("SPIx Recv: %d", sum);
    return( sum );
}
...
static int CLI_DeviceOpen(cli_device_t* device, int cli)
{
    ...
    if (SPI_Init() < 0) {
        LOG_ERROR("CLI \"" CMD_CLI_NAME "\" fault on Init the SPI" TO_STRING(SPIx_DEVICE_NUM));
        return( -1 );
    }

    if (SPIx_FORWARD_IDX >= 0) {      //lint !e506 !e774
        LOG_INFO("CLI \"" CMD_CLI_NAME "\" Opened on the SPI" TO_STRING(SPIx_DEVICE_NUM)
                 ", Forward Dst on Idx: %d", (int)SPIx_FORWARD_IDX
                );
        Cmd_ForwardSetIF((unsigned)SPIx_FORWARD_IDX, cli);          //lint !e534
    }
    else if (SPIx_FORWARD_SRC >= 0) { //lint !e506 !e774
        LOG_INFO("CLI \"" CMD_CLI_NAME "\" Opened on the SPI" TO_STRING(SPIx_DEVICE_NUM)
                 ", Default Src on Idx: %d", (int)SPIx_FORWARD_SRC
                );
        Cmd_ForwardToIdx((unsigned)SPIx_FORWARD_SRC, cli, 0, NULL); //lint !e534
    }
    else {
        LOG_INFO("CLI \"" CMD_CLI_NAME "\" Opened on the SPI" TO_STRING(SPIx_DEVICE_NUM));
    }
...
static int CLI_DeviceClose(cli_device_t* device, int cli)
{
    ...
    LOG_ERROR("CLI \"" CMD_CLI_NAME "\" Closed on the SPI" TO_STRING(SPIx_DEVICE_NUM));
    return( 0 );
}
...
static int CLI_DeviceRead(cli_device_t* device, void* buff, int size)
{
    ...
    if((buff == NULL) || (size <= 0)) {
        LOG_ERROR("CLI \"" CMD_CLI_NAME "\" Read fault on the SPI" TO_STRING(SPIx_DEVICE_NUM)
...
static int CLI_DeviceWrite(cli_device_t* device, const void* data, int size)
{
    ...
    if((data == NULL) || (size <= 0)) {
        LOG_ERROR("CLI \"" CMD_CLI_NAME "\" Write fault on the SPI" TO_STRING(SPIx_DEVICE_NUM)
...
int Cmd_SPIxInit(void)
{
    ...
    int  flags = CMD_CLI_FLAGS(SPIx_CMD_STR_EN, SPIx_CMD_BIN_EN);
    assert((SPIx_CMD_STR_EN + SPIx_CMD_BIN_EN) > 0);

    if (cli_shell_new(&_cli_device, CMD_CLI_NAME, flags) < 0) {
