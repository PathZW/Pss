 ...
 * @version v1.0.0
 * @date    2019/6/29
 * @brief   Command line interface on RS232.
 *******************************************************************************
 * @attention
 *
 ...
 * @addtogroup __Config
 * @{
 * @addtogroup          Cmd_RS232
 * @{
 ******************************************************************************/

#ifndef RS232_USART_NUM
#define RS232_USART_NUM           -1        //!< (C) USART number, ( < 0) Disable
#endif
#ifndef RS232_BAUDRATE
#define RS232_BAUDRATE           115200     //!< (R) USART baudrate
#endif
#ifndef RS232_BUFF_SIZE
#define RS232_BUFF_SIZE           128       //!< (C) USART buffer size
#endif

#ifndef RS232_FLOW_CONTROL
#define RS232_FLOW_CONTROL        -1        //!< (R) USART Flow Control: (-1)None, (0)Custom, (1)Custom/CTS, (2)RTS, (3)RTS/CTS
#endif
#ifndef RS232_FLOW_CTRL_INIT
#define RS232_FLOW_CTRL_INIT()  ((void)0)   //!< (C) USART Custom Flow Control Initialize, default Soft-Mode RTS
#endif
#ifndef RS232_FLOW_RTS_SET
#define RS232_FLOW_RTS_SET()  USARTx_SetRTS //!< (C) USART Custom Flow Control Activate RTS(low), default Soft-Mode RTS
#endif
#ifndef RS232_FLOW_RTS_CLR
#define RS232_FLOW_RTS_CLR()  USARTx_ClrRTS //!< (C) USART Custom Flow Control Deactivate RTS(high), default Soft-Mode RTS
#endif

#ifndef RS232_CMD_STR_EN
#define RS232_CMD_STR_EN           1        //!< (R) 是(1)/否(0) 支持字符串命令
#endif
#ifndef RS232_CMD_BIN_EN
#define RS232_CMD_BIN_EN           0        //!< (R) 是(1)/否(0) 支持二进制命令
#endif
#ifndef RS232_FORWARD_IDX
#define RS232_FORWARD_IDX         -1        //!< (R) 字符串命令转发: (< 0)非转发接口, (>= 0)转发接口索引
#endif
#ifndef RS232_FORWARD_SRC
#define RS232_FORWARD_SRC         -1        //!< (R) 命令转发源接口: (< 0)非源接口, (>= 0)源接口索引
#endif
...
#define USARTx_CONTROLLER                           RS232_USART_NUM
#define USARTx_BAUDRATE         /*lint -save -w1 */ RS232_BAUDRATE         /*lint -restore */
#define USARTx_BUF_SIZE         /*lint -save -w1 */ RS232_BUFF_SIZE        /*lint -restore */

#define USARTx_FLOW_CONTROL     /*lint -save -w1 */ RS232_FLOW_CONTROL     /*lint -restore */
#define USARTx_FLOW_CTRL_INIT() /*lint -save -w1 */ RS232_FLOW_CTRL_INIT() /*lint -restore */
#define USARTx_FLOW_RTS_SET()   /*lint -save -w1 */ RS232_FLOW_RTS_SET()   /*lint -restore */
#define USARTx_FLOW_RTS_CLR()   /*lint -save -w1 */ RS232_FLOW_RTS_CLR()   /*lint -restore */

#define CMD_CLI_STR_EN          /*lint -save -w1 */ RS232_CMD_STR_EN       /*lint -restore */
#define CMD_CLI_BIN_EN          /*lint -save -w1 */ RS232_CMD_BIN_EN       /*lint -restore */
#define CMD_FORWARD_IDX         /*lint -save -w1 */ RS232_FORWARD_IDX      /*lint -restore */
#define CMD_FORWARD_SRC         /*lint -save -w1 */ RS232_FORWARD_SRC      /*lint -restore */
...
/*----------------------------------------------------------------------------*/
/**
 * Initialize the RS232 application
 */
void Cmd_RS232Init(void)
{
    ...
    if (cli_shell_new(&_cli_device, "RS232", flags) < 0) {
        assert_msg(0, "ERROR: cli_shell_new(&_cli_device, \"RS232\", flags)");
    }
}
