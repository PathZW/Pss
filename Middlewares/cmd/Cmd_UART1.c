 ...
 * @version v1.0.0
 * @date    2019/6/29
 * @brief   Command line interface on UART1.
 *******************************************************************************
 * @attention
 *
 ...
 * @addtogroup __Config
 * @{
 * @addtogroup          Cmd_UART1
 * @{
 ******************************************************************************/

#ifndef UART1_DEVICE_NUM
#define UART1_DEVICE_NUM          -1        //!< (C) USART number, ( < 0) Disable
#endif
#ifndef UART1_BAUDRATE
#define UART1_BAUDRATE           115200     //!< (R) USART baudrate
#endif
#ifndef UART1_BUFF_SIZE
#define UART1_BUFF_SIZE           128       //!< (C) USART buffer size
#endif

#ifndef UART1_FLOW_CONTROL
#define UART1_FLOW_CONTROL        -1        //!< (R) USART Flow Control: (-1)None, (0)Custom, (1)Custom/CTS, (2)RTS, (3)RTS/CTS
#endif
#ifndef UART1_FLOW_CTRL_INIT
#define UART1_FLOW_CTRL_INIT()  ((void)0)   //!< (C) USART Custom Flow Control Initialize, default Soft-Mode RTS
#endif
#ifndef UART1_FLOW_RTS_SET
#define UART1_FLOW_RTS_SET()  USARTx_SetRTS //!< (C) USART Custom Flow Control Activate RTS(low), default Soft-Mode RTS
#endif
#ifndef UART1_FLOW_RTS_CLR
#define UART1_FLOW_RTS_CLR()  USARTx_ClrRTS //!< (C) USART Custom Flow Control Deactivate RTS(high), default Soft-Mode RTS
#endif

#ifndef UART1_CMD_STR_EN
#define UART1_CMD_STR_EN           1        //!< (R) 是(1)/否(0) 支持字符串命令
#endif
#ifndef UART1_CMD_BIN_EN
#define UART1_CMD_BIN_EN           0        //!< (R) 是(1)/否(0) 支持二进制命令
#endif
#ifndef UART1_FORWARD_IDX
#define UART1_FORWARD_IDX         -1        //!< (R) 字符串命令转发: (< 0)非转发接口, (>= 0)转发接口索引
#endif
#ifndef UART1_FORWARD_SRC
#define UART1_FORWARD_SRC         -1        //!< (R) 命令转发源接口: (< 0)非源接口, (>= 0)源接口索引
#endif
...
#define USARTx_CONTROLLER                           UART1_DEVICE_NUM
#define USARTx_BAUDRATE         /*lint -save -w1 */ UART1_BAUDRATE         /*lint -restore */
#define USARTx_BUF_SIZE         /*lint -save -w1 */ UART1_BUFF_SIZE        /*lint -restore */

#define USARTx_FLOW_CONTROL     /*lint -save -w1 */ UART1_FLOW_CONTROL     /*lint -restore */
#define USARTx_FLOW_CTRL_INIT() /*lint -save -w1 */ UART1_FLOW_CTRL_INIT() /*lint -restore */
#define USARTx_FLOW_RTS_SET()   /*lint -save -w1 */ UART1_FLOW_RTS_SET()   /*lint -restore */
#define USARTx_FLOW_RTS_CLR()   /*lint -save -w1 */ UART1_FLOW_RTS_CLR()   /*lint -restore */

#define CMD_CLI_STR_EN          /*lint -save -w1 */ UART1_CMD_STR_EN       /*lint -restore */
#define CMD_CLI_BIN_EN          /*lint -save -w1 */ UART1_CMD_BIN_EN       /*lint -restore */
#define CMD_FORWARD_IDX         /*lint -save -w1 */ UART1_FORWARD_IDX      /*lint -restore */
#define CMD_FORWARD_SRC         /*lint -save -w1 */ UART1_FORWARD_SRC      /*lint -restore */
...
/*----------------------------------------------------------------------------*/
/**
 * Initialize the UART1 application
 */
void Cmd_UART1Init(void)
{
    ...
    if (cli_shell_new(&_cli_device, "UART1", flags) < 0) {
        assert_msg(0, "ERROR: cli_shell_new(&_cli_device, \"UART1\", flags)");
    }
}
