...
/* AD5683命令 */
#define _CMD_WRITE_INPUT_REG_                        (0x01) //通过Command2可以写入DAC_REG
#define _CMD_UPDATA_DAC_REG_                         (0x02)
#define _CMD_WRITE_DAC_REG_                          (0x03)
#define _CMD_WRITE_CTRL_REG_                         (0x04)
#define _CMD_RDBCK_INPUT_REG_                        (0x05)

#define DEV_AD5683_FIFOISFULL          ((dev->devRegs->fifoStateReg & 0x1) == 0x1)    
#define DEV_AD5683_FIFOISNULL          ((dev->devRegs->fifoStateReg & 0x2) == 0x2)
#define DEV_AD5683_FIFOISBUSY          ((dev->devRegs->fifoStateReg & 0x4) == 0x4)

/*Data Source*/
#define DEV_AD5683Send_TO_CMD_REG    0
#define DEV_AD5683Send_TO_FIFO      1
/*Data Source*/

/* Set VAL*/
#define DEV_AD5683_SETVAL_NOPULSE       0
#define DEV_AD5683_SETVAL_ISPULSE       1

#define DEV_AD5683_HIGHVAL_VALUE(val)         (val*26214)											
#define DEV_AD5683_VAL_ESTIMATE(val) ((val < 0)?0:(DEV_AD5683_HIGHVAL_VALUE(MIN(val,2.5))))                                             

/*regVal最后写入REG的值，chipNum通道号，val写入的原始值，REG最终写入的寄存器*/
#define DEV_AD5683_WRITE_INPUTREG_UPDATA(chipNum,val,REG) \
do{ \
	uint32_t regVal; \
	regVal = val; \
	regVal <<= 4; \
	regVal |= (chipNum << PSS_MCU_DRVS_AD5683_NUM_CTRL_OFFSET); \
	regVal |= (_CMD_WRITE_DAC_REG_ << 20); \
	REG = regVal; \
}while(0)
...
/*********************************** 类型定义 **********************************/
typedef struct {
	volatile  			uint32_t 		fifoReg;        //00 FIFO寄存器
	volatile  			uint32_t 		fifoStateReg;   //04 FIFO状态寄存器    
	volatile  			uint32_t 		cmdReg;         //08 命令寄存器
	volatile  			uint32_t 		resetReg;       //0c 复位寄存器 

	volatile const  uint32_t    mCurData;       // 10 当前数据; BIT[23..0]:数据; BIT24:极性 r
	volatile        uint32_t    mPolarity;      // 14 极性选择; (x)从CHIPx获取极性 w
	volatile        uint32_t    mDataFrom;      // 18 输出数据来源; BIT0:(0)输出指令,(1)其他 w
	volatile        uint32_t    mFIFOvldNum;    // 1C FIFO 有效数据个数 w
} FpgaAD5683_REGS;
