...
#define  DEV_DRIVER_INFO_USED   // 启用 xxxx_INFO
...
static int32_t devCtrl(uint32_t *cmd, uint32_t opt, va_list args, FpgaAD5761_RESOURCE *dev)
{
    ...
    {   //---------------------- 设备电源打开 ---------------------------
        ...
        DEV_EVT_CODE_SET(DEV_EVT_POWER_CTRL, 1)     //!< 设置事件码
    }
    ...
    {   //---------------------- 设备电源关闭 ---------------------------
        ...
        DEV_EVT_CODE_SET(DEV_EVT_POWER_CTRL, 0)     //!< 设置事件码
    }
...
//  DEV_CMD_BIT_CODE_END(DEV_CMD_DAC_xxxx, *cmd)
    ...
    {   //---------------------- 终止发送 -------------------------------
        ...
        DEV_EVT_CODE_SET(DEV_EVT_ABORT_SEND, 0)     //!< 设置事件码
    }
    ...
    {   //---------------------- 终止接收 -------------------------------
        ...
        DEV_EVT_CODE_SET(DEV_EVT_ABORT_RECV, 0)     //!< 设置事件码
    }
